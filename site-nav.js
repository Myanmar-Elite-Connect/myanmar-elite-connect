...
(function(){
  ...
  const remove=()=>document.querySelectorAll('.mobile-nav,.bottom-nav,.bottom-navigation,.site-bottom-nav,#bottomNav,#mec-global-nav').forEach(el=>el.remove());
  remove();
  ...
  nav.innerHTML=items.map(([href,icon,label])=>
    ...
    `<span class="mec-nav-icon">${icon}</span><span class="mec-nav-label">${label}</span></a>`
  ).join('');
...
#mec-global-nav{
  ...
  z-index:1100;width:min(560px,calc(100% - 20px));min-height:68px;padding:7px;
  display:grid;grid-template-columns:repeat(4,1fr);gap:5px;
  background:var(--card,#fff);border:1px solid var(--line,rgba(16,35,63,.10));
...
#mec-global-nav a:hover{color:var(--text,#10233f);background:var(--soft,rgba(16,35,63,.05))}

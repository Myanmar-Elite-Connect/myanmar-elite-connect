...
(function(){
  ...
  const removeExistingNav = () => {
    document.querySelectorAll('.mobile-nav,.bottom-nav,.bottom-navigation,.site-bottom-nav,#bottomNav').forEach(el=>el.remove());
  };
  removeExistingNav();
  ...
  nav.innerHTML = items.map(([href,icon,label]) => `
    <a href="./${href}" class="${current===href?'active':''}" aria-label="${label}">
      ...
      <span class="mec-nav-label">${label}</span>
    </a>`).join('');
  ...
  style.textContent = `
    #mec-global-nav{
      ...
      z-index:99999;width:min(560px,calc(100% - 20px));min-height:68px;padding:7px;
      display:grid;grid-template-columns:repeat(4,1fr);gap:5px;
      background:color-mix(in srgb,var(--surface,#fff) 94%,transparent);
      border:1px solid var(--border,rgba(16,35,63,.09));border-radius:20px;
      box-shadow:0 14px 45px rgba(13,35,64,.16);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);
      padding-bottom:max(7px,env(safe-area-inset-bottom));
    }
    #mec-global-nav a{
      min-width:0;min-height:52px;display:flex;flex-direction:column;align-items:center;justify-content:center;
      gap:3px;border-radius:14px;color:var(--muted,#718097);text-decoration:none;font-size:9px;font-weight:900;
  ...
  const mount=()=>{removeExistingNav();document.head.appendChild(style);document.body.appendChild(nav)};

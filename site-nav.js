...
(function(){
  ...
  style.textContent=`
/* Shared MEC navigation — follows each page's existing light/dark theme variables. */
#mec-global-nav{
  --mec-nav-bg:var(--surface,var(--card,#fff));
  --mec-nav-border:var(--border,var(--line,rgba(16,35,63,.10)));
  --mec-nav-text:var(--text,#10233f);
  --mec-nav-muted:var(--muted,#718097);
  --mec-nav-soft:var(--surface-2,var(--soft,rgba(16,35,63,.05)));
  --mec-nav-gold:var(--gold,#d8b45c);
  ...
  background:var(--mec-nav-bg);border:1px solid var(--mec-nav-border);
  border-radius:20px;box-shadow:var(--shadow,var(--floating-shadow,0 14px 45px rgba(16,35,63,.12)));
...
#mec-global-nav a{
  ...
  gap:3px;border-radius:14px;color:var(--mec-nav-muted);text-decoration:none;font-size:9px;font-weight:900;
